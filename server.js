...
(function() {
  ...
  var _checkInterval = setInterval(function() {
    ...
    window.raGenerateReceiptPDF = function(bookingId, invoiceIdx) {
      ...
      var inv = invoices ? invoices[invoiceIdx] : null;
      var b = (typeof allBookings !== 'undefined') ? allBookings.find(function(x){return String(x.id)===String(bookingId);}) : null;
      var nm = b ? (b.name||b.guest_name||'customer').replace(/\\s+/g,'_') : 'customer';
      var typeFile = (inv && inv.type==='deposit') ? 'receipt_deposit' : 'receipt_rent';
      var safeDate = (inv && inv.date ? inv.date : new Date().toLocaleDateString('en-GB')).replace(/\\//g,'-');
      var fileName = typeFile + '_' + nm + '_' + safeDate + '.png';
      
      // Fetch receipt image from server
      fetch(SERVER + '/generate-receipt-image/' + bookingId + '/' + invoiceIdx)
        .then(function(r) {
        ...
        .catch(function(err) {
          ...
          if(typeof raToast==='function') raToast('\\u274c \\u0641\\u0634\\u0644 \\u062a\\u0648\\u0644\\u064a\\u062f \\u0627\\u0644\\u0625\\u064a\\u0635\\u0627\\u0644: ' + err.message, 'error');
        });
    };
  ...
  window._raViewReceipts = function(bookingId) {
    ...
    if (invoices.length === 0) {
      ...
      invoices.forEach(function(inv, idx) {
        ...
        content += '<button onclick="window._raViewReceiptImage(\\''+bookingId+'\\','+idx+')" style="background:#5a4a3a;color:#fff;border:none;padding:6px 10px;border-radius:6px;cursor:pointer;font-size:0.8rem;">\\ud83d\\udc41 \\u0645\\u0634\\u0627\\u0647\\u062f\\u0629</button>';
        content += '<button onclick="window.raGenerateReceiptPDF(\\''+bookingId+'\\','+idx+')" style="background:#2d6a4f;color:#fff;border:none;padding:6px 10px;border-radius:6px;cursor:pointer;font-size:0.8rem;">\\ud83d\\udce5 \\u062a\\u062d\\u0645\\u064a\\u0644</button>';
        content += '</div>';
        content += '</div>';
        if (inv.notes) {
  ...
  window._raViewReceiptImage = function(bookingId, invoiceIdx) {
    ...
    overlay.innerHTML = '<div style="text-align:center;color:#fff;font-family:Tajawal,Arial,sans-serif;"><p>\u062c\u0627\u0631\u064a \u062a\u0648\u0644\u064a\u062f \u0627\u0644\u0625\u064a\u0635\u0627\u0644...</p></div>';
    document.body.appendChild(overlay);
    ...
    img.onload = function() {
      overlay.innerHTML = '<div style="background:#fff;border-radius:12px;max-width:550px;width:100%;max-height:90vh;overflow:auto;padding:16px;position:relative;direction:rtl;font-family:Tajawal,Arial,sans-serif;">'
        + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">'
        + '<h3 style="margin:0;font-size:1rem;color:#1B4332;">\ud83e\uddfe \u0627\u0644\u0625\u064a\u0635\u0627\u0644</h3>'
        + '<button onclick="document.getElementById(\'ra-receipt-image-overlay\').remove()" style="background:none;border:none;font-size:1.5rem;cursor:pointer;color:#999;">&times;</button>'
        + '</div>'
        + '<img src="' + imgUrl + '" style="width:100%;border-radius:8px;border:1px solid #ddd;" />'
        + '<div style="display:flex;gap:8px;margin-top:10px;">'
        + '<button onclick="window.raGenerateReceiptPDF(\''+bookingId+'\','+invoiceIdx+')" style="flex:1;padding:10px;background:#2d6a4f;color:#fff;border:none;border-radius:6px;font-size:0.85rem;font-weight:600;cursor:pointer;">\ud83d\udce5 \u062a\u062d\u0645\u064a\u0644</button>'
        + '<button onclick="document.getElementById(\'ra-receipt-image-overlay\').remove()" style="flex:1;padding:10px;background:#7a6b5a;color:#fff;border:none;border-radius:6px;font-size:0.85rem;font-weight:600;cursor:pointer;">\u2716 \u0625\u063a\u0644\u0627\u0642</button>'
        + '</div>'
        + '</div>';
    };
    img.onerror = function() {
      overlay.innerHTML = '<div style="background:#fff;border-radius:12px;max-width:400px;width:100%;padding:30px;text-align:center;direction:rtl;font-family:Tajawal,Arial,sans-serif;">'
        + '<p style="font-size:2rem;">\u274c</p>'
        + '<p>\u0641\u0634\u0644 \u062a\u0648\u0644\u064a\u062f \u0627\u0644\u0625\u064a\u0635\u0627\u0644</p>'
        + '<button onclick="document.getElementById(\'ra-receipt-image-overlay\').remove()" style="margin-top:10px;padding:8px 20px;background:#7a6b5a;color:#fff;border:none;border-radius:6px;cursor:pointer;">\u0625\u063a\u0644\u0627\u0642</button>'
        + '</div>';
    };
    img.src = imgUrl;
  };
